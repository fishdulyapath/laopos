...
router.get('/validatecartstock', async (req, res) => {
  ...
  const sql = `
    WITH cart AS (
      ...
      LEFT JOIN ic_inventory i ON i.code = c.item_code
      WHERE c.cust_code = $1
      GROUP BY c.item_code, c.item_name, c.unit_code, COALESCE(i.item_type, 0)
    ),
    normal_codes AS (
    ...
    normal_stock AS (
      ...
      FROM (SELECT codes FROM normal_codes WHERE codes IS NOT NULL AND codes <> '') nc
      CROSS JOIN LATERAL sml_ic_function_stock_balance_warehouse_location(
        current_date, nc.codes, '', ''
      ) s
    ...
    set_detail AS (
      ...
      WHERE d.ic_set_code IN (SELECT item_code FROM cart WHERE item_type = 3)
    ),
    ...
    set_stock AS (
      SELECT
        ic_set_code,
        MIN(TRUNC(sum_balance_qty / NULLIF(qty, 0))) AS balance_qty
    ...
    SELECT
      ...
      CASE
        ...
        ELSE COALESCE(TRUNC(COALESCE(ns.balance_qty, 0) / COALESCE(NULLIF(u.ratio, 0), 1)), 0)
      END AS balance_qty
    FROM cart c
    LEFT JOIN normal_stock ns ON ns.ic_code = c.item_code
    LEFT JOIN set_stock ss ON ss.ic_set_code = c.item_code
  ...
  try {
    ...
    for (const r of result.rows) {
      const qtyInCart = parseFloat(r.qty_in_cart) || 0;
      const balanceQty = parseFloat(r.balance_qty) || 0;

      if (balanceQty <= 0 || qtyInCart > balanceQty) {
        stockIssues.push({
          item_code: r.item_code,
          item_name: r.item_name,
