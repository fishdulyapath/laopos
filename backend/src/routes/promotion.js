...
function promotionActionEffect(command) {
  const raw = String(command?.Command ?? command?.command ?? '').trim();
  const value = raw.startsWith('=') ? raw.slice(1).trim() : raw;
...
function buildPromotionDetails(promotions, appliedCodes, itemNameByCode, cartQtyMaps, promotionFixedUnitCode) {
  ...
  for (const promotion of promotions || []) {
    ...
    const actionEffects = (promotion.Actions || [])
      .map((action) => promotionActionEffect(action))
      .filter(Boolean);
...
function mapPromotionHintRows(items, rows, promotionDetails = new Map()) {
  ...
  for (const row of rows || []) {
    ...
    target.promotions.push({
      ...
      action_effects: details.action_effects || [],
    });
  }
