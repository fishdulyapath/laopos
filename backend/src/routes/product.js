...
const { getProductPriceLocalx, getPricePos } = require("../utils/priceHelper");
...
const SALE_STOCK_ADJUST_DOC_FORMAT_CODE = "IAPOS";
...
function roundStockAmount(value, digits = 6) {
...
async function insertExistingColumns(client, tableName, data) {
  ...
  if (columns.has("create_date_time_now") && (insertData.create_date_time_now === undefined || insertData.create_date_time_now instanceof Date)) {
    insertData.create_date_time_now = bangkokTimestamp(insertData.create_date_time_now);
  }
  const entries = Object.entries(insertData).filter(([column]) => columns.has(column));
  ...
  return client.query(`INSERT INTO ${tableName} (${columnSql}) VALUES (${valueSql})`, values);
}

async function generateStockRunningDocNo(client, prefix) {
  await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [`ic_trans:${prefix}`]);
  const result = await client.query(
    `SELECT doc_no
     FROM ic_trans
     WHERE doc_no LIKE $1
  ...
  const lastRunning = result.rows.length > 0 ? parseInt(String(result.rows[0].doc_no || "").slice(-4), 10) : 0;
  return `${prefix}${String((Number.isFinite(lastRunning) ? lastRunning : 0) + 1).padStart(4, "0")}`;
}

async function resolveStockAdjustmentDoc(client, screenCode, transFlag, docDate, docFormatCode = "") {
  const screen = cleanText(screenCode);
  const formatCode = cleanText(docFormatCode || screen);
  await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [`ic_trans:${screen}:${formatCode}:${transFlag}:${docDate}`]);
  try {
    return await resolveDocumentNo(client, {
      screenCode: screen,
      docFormatCode: formatCode,
      transFlag,
      docDate,
    });
  } catch (error) {
    if (formatCode !== screen) throw error;
    if (!String(error.message || "").includes("doc_format_code not found")) throw error;
    return resolveDocumentNo(client, {
      screenCode: screen,
      docFormatCode: "",
      transFlag,
      docDate,
...
async function verifySaleStockAdjustUser(client, userCode, password) {
  const result = await client.query(
    `SELECT code AS user_code, name_1 AS user_name, COALESCE(price_level_1, 0) AS price_level_1
     ...
     ORDER BY code
     LIMIT 1`,
    [cleanText(userCode), String(password ?? "")],
  );
  ...
  if (safeNumber(row.price_level_1) !== 1) {
    const error = new Error("user does not have price edit permission");
    error.statusCode = 403;
    throw error;
  }
  return {
    user_code: cleanText(row.user_code),
    user_name: cleanText(row.user_name),
  };
}

async function loadPosStockLocation(client, posId) {
  const code = cleanText(posId);
  ...
  const row = result.rows[0] || {};
  return {
    wh_code: cleanText(row.wh_code),
    shelf_code: cleanText(row.shelf_code),
  };
}

async function loadStockAdjustUnitInfo(client, itemCode, unitCode) {
  const result = await client.query(
    `SELECT COALESCE(ratio, 0)::numeric AS ratio,
            COALESCE(stand_value, 1)::numeric AS stand_value,
            COALESCE(divide_value, 1)::numeric AS divide_value
     FROM ic_unit_use
     WHERE ic_code = $1
       AND code = $2
     LIMIT 1`,
  ...
  const row = result.rows[0] || {};
  const standValue = safeNumber(row.stand_value, 1) || 1;
  const divideValue = safeNumber(row.divide_value, 1) || 1;
  return {
    stand_value: standValue,
    divide_value: divideValue,
    ratio: safeNumber(row.ratio) || standValue / divideValue || 1,
  };
}

async function loadStockAdjustAverageCost(client, docDate, itemCode, whCode, shelfCode) {
  const result = await client.query(
    `SELECT COALESCE(MAX(average_cost_end), 0)::numeric AS average_cost
     FROM sml_ic_function_stock_balance_warehouse_location($1::date, $2, $3, $4)`,
    [docDate, itemCode, whCode || "", shelfCode || ""],
  );
  return safeNumber(result.rows[0]?.average_cost);
}
...
router.get("/getProductList", async (req, res) => {
  ...
  try {
    ...
    const dataSQL =
      ...
      ` COALESCE(c.is_premium,0) AS is_premium,` +
...
router.get("/getProductBarcodeSearch", async (req, res) => {
  const {
    ...
    offset: strOffset = "0",
    limit: strLimit = "50",
    include_balance: strIncludeBalance = "",
    wh_code: strWhCode = "",
    shelf_code: strShelfCode = "",
  } = req.query;
  const resp = { success: false };
  ...
  try {
    ...
    const includeBalance = ["1", "true", "yes"].includes(String(strIncludeBalance || "").toLowerCase());
    let whParam = "''";
    let shelfParam = "''";
    if (includeBalance) {
      params.push(String(strWhCode || "").trim());
      whParam = `$${params.length}`;
      params.push(String(strShelfCode || "").trim());
      shelfParam = `$${params.length}`;
    }
    const balanceSelect = includeBalance
      ? `
      SELECT
        paged.*,
        COALESCE(TRUNC(COALESCE(stock.sum_balance_qty, 0) / NULLIF(paged.ratio, 0), 0), 0) AS balance_qty,
        COALESCE(stock.sum_balance_qty, 0) AS sum_balance_qty
      FROM paged
      LEFT JOIN LATERAL (
        SELECT COALESCE(SUM(s.balance_qty), 0) AS sum_balance_qty
        FROM sml_ic_function_stock_balance_warehouse_location('NOW()', paged.item_code, ${whParam}, ${shelfParam}) s
        WHERE s.balance_qty > 0
      ) stock ON TRUE
      ORDER BY paged.item_code, paged.unit_code, paged.barcode`
      : `
      SELECT
        paged.*,
        NULL::numeric AS balance_qty,
        NULL::numeric AS sum_balance_qty
      FROM paged
      ORDER BY paged.item_code, paged.unit_code, paged.barcode`;
    ...
    const data = result.rows.map((r) => ({
      ...
      sum_balance_qty: r.sum_balance_qty == null ? null : Number(r.sum_balance_qty),
    }));
  ...
  try {
    ...
    for (const r of result.rows) {
      ...
      try {
        ...
        const priceRes = await getProductPriceLocalx(r.ic_code, r.unit_code, "1", strCustCode, vatType, vatRate, saleType, r.barcode, docDate, strCurrencyCode);
  ...
  try {
    ...
    for (const r of result.rows) {
      ...
      try {
        ...
        const prices = await getProductPriceLocalx(r.ic_code, r.unit_code, "1", strCust, vatType, vatRate, saleType, r.barcode, docDate, strCurrencyCode);
...
router.get("/getProductPrice", async (req, res) => {
  ...
  try {
    ...
    const result = await getProductPriceLocalx(strItemCode, strUnitCode, strQty || "1", strCustCode, vatType, vatRate, saleType, barcodeForPrice, docDate, strCurrencyCode);
    resp.success = true;
...
// GET /service/v1/getpricepos
// ดึงราคาแบบ POS C#: barcode/member/promotion ก่อน แล้วค่อย fallback ไป _findPriceByItem แบบภาษีรวมใน
router.get("/getpricepos", async (req, res) => {
  const {
    item_code: strItemCode = "",
    unit_code: strUnitCode = "",
    qty: strQty = "1",
    cust_code: strCustCode = "",
    member_code: strMemberCode = "",
    barcode: strBarcode = "",
    price_number: strPriceNumber = "1",
    is_staff: strIsStaff = "",
    pos_id: strPosId = "",
  ...
  try {
    const result = await getPricePos({
      ...
      barcode: strBarcode,
      priceNumber: parseInt(strPriceNumber, 10) || 1,
      isStaff: strIsStaff,
      posId: strPosId,
      branchCode: strBranchCode,
      defaultCustCode: strDefaultCustCode,
      docDate: strDocDate.trim() || undefined,
...
router.get("/getProductByBarcodeDetail", async (req, res) => {
  const { barcode: strBarcode = "", wh_code: strWhCode = "", shelf_code: strShelfCode = "" } = req.query;
  const resp = { success: false };

  const scanCode = strBarcode.trim().toUpperCase();
  const posWhCode = strWhCode.trim();
  const posShelfCode = strShelfCode.trim();
  ...
  try {
      ...
      `WITH candidate AS (
         ...
         FROM (
           SELECT
             ...
             'barcode' AS scan_source,
             b.ic_code AS item_code,
             COALESCE(b.unit_code, '') AS barcode_unit_code,
             b.barcode,
             NULL::numeric AS serial_status,
             ''::text AS serial_wh_code,
             ''::text AS serial_shelf_code,
         ...
         FROM (
           SELECT
             ...
             'serial' AS scan_source,
             s.ic_code AS item_code,
             COALESCE(s.ic_unit, '') AS barcode_unit_code,
             COALESCE((
               SELECT ib.barcode
               FROM ic_inventory_barcode ib
               WHERE ib.ic_code = s.ic_code AND ib.unit_code = s.ic_unit
               LIMIT 1
             ), '') AS barcode,
             COALESCE(NULLIF(s.status::text, '')::numeric, 0) AS serial_status,
             COALESCE(s.wh_code, '') AS serial_wh_code,
             COALESCE(s.shelf_code, '') AS serial_shelf_code,
             s.serial_number
           FROM ic_serial s
         ...
         FROM (
           SELECT
             ...
             'item_code' AS scan_source,
             i.code AS item_code,
             '' AS barcode_unit_code,
             i.code AS barcode,
             NULL::numeric AS serial_status,
             ''::text AS serial_wh_code,
             ''::text AS serial_shelf_code,
       ...
       barcode_row AS (
         SELECT
           ...
           d.start_sale_shelf
         FROM candidate c
         JOIN ic_inventory i ON i.code = c.item_code
         LEFT JOIN ic_inventory_detail d ON d.ic_code = i.code
         LIMIT 1
       ),
       resolved AS (
    ...
    const row = result.rows[0];
    if (row.scan_source === "serial") {
      if (Number(row.serial_status || 0) === 1) {
        return res.status(400).json({ ERROR: `Serial already sold: ${scanCode}` });
      }
      if (posWhCode && String(row.serial_wh_code || "") !== posWhCode) {
        return res.status(400).json({ ERROR: `Serial warehouse does not match POS warehouse: ${scanCode}` });
      }
      if (posShelfCode && String(row.serial_shelf_code || "") !== posShelfCode) {
        return res.status(400).json({ ERROR: `Serial shelf does not match POS shelf: ${scanCode}` });
      }
    }
  ...
  try {
    ...
    await withTransaction(async (client) => {
      ...
      if (diff_qty !== 0) {
        ...
        const adjScreenCode = diff_qty > 0 ? "IA" : "IS";
        const adjDoc = await resolveStockAdjustmentDoc(client, adjScreenCode, adj_flag, doc_date);
        doc_no_adj = adjDoc.doc_no;
        ...
        await client.query(
          ...
          [adj_flag, doc_no_adj, doc_date, doc_time, adjDoc.doc_format_code, branch_code, wh_code, shelf_code, doc_no],
        );
        await client.query(
...
// POST /service/v1/adjustStockFromSale
// ปรับยอดสต๊อกจริงจากหน้าขายเมื่อสต๊อก POS เป็นศูนย์: ใช้สิทธิ์เดียวกับแก้ไขราคา
router.post("/adjustStockFromSale", async (req, res) => {
  const {
    item_code = "",
    item_name = "",
    unit_code = "",
  ...
  const itemCode = cleanText(item_code);
  const unitCode = cleanText(unit_code);
  const targetQty = roundStockAmount(qty);
  if (!itemCode || !unitCode || !Number.isFinite(targetQty) || targetQty < 0) {
    return res.status(400).json({ success: false, msg: "item_code, unit_code and qty are required" });
  }

  try {
    const data = await withTransaction(async (client) => {
      const approver = await verifySaleStockAdjustUser(client, user_code, password);
      const posLocation = await loadPosStockLocation(client, pos_id);
      const finalWhCode = cleanText(posLocation.wh_code || wh_code);
      const finalShelfCode = cleanText(posLocation.shelf_code || shelf_code);
      if (!finalWhCode) {
        const error = new Error("POS warehouse is required");
        error.statusCode = 400;
        throw error;
      }

      const now = new Date();
      const docDate = formatLocalDate(now);
      const docTime = formatLocalTime(now);
      const remark = cleanText(pos_code || pos_id);
      const unit = await loadStockAdjustUnitInfo(client, itemCode, unitCode);
      const ratio = unit.ratio || 1;
      const balanceResult = await client.query(
        `SELECT COALESCE(SUM(balance_qty), 0)::numeric AS sum_balance_qty
         FROM sml_ic_function_stock_balance_warehouse_location($1::date, $2, $3, $4)`,
        [docDate, itemCode, finalWhCode, finalShelfCode],
      );
      const balanceBaseQty = safeNumber(balanceResult.rows[0]?.sum_balance_qty);
      const currentQty = roundStockAmount(balanceBaseQty / ratio);
      const diffQty = roundStockAmount(targetQty - currentQty);
      if (diffQty <= 0) {
        const error = new Error("target stock must be greater than current stock for stock increase");
        error.statusCode = 400;
        throw error;
      }
      const averageCost = await loadStockAdjustAverageCost(client, docDate, itemCode, finalWhCode, finalShelfCode);
      const diffCost = roundStockAmount(diffQty * ratio * averageCost, 2);
      const finalBranchCode = cleanText(branch_code);
      const finalItemName = cleanText(item_name);
      const adjDoc = await resolveStockAdjustmentDoc(client, "IA", 66, docDate, SALE_STOCK_ADJUST_DOC_FORMAT_CODE);

      await insertExistingColumns(client, "ic_trans", {
        trans_flag: 66,
        trans_type: 3,
        doc_no: adjDoc.doc_no,
        doc_date: docDate,
        doc_time: docTime,
        doc_format_code: adjDoc.doc_format_code,
        doc_ref: approver.user_code,
        doc_ref_date: docDate,
        remark,
        branch_code: finalBranchCode,
        wh_from: finalWhCode,
        location_from: finalShelfCode,
        total_amount: diffCost,
        total_amount_2: diffCost,
        total_value: diffCost,
        creator_code: approver.user_code,
        sale_code: approver.user_code,
      });
      ...
      await insertExistingColumns(client, "ic_trans_detail", {
        ...
        doc_date: docDate,
        doc_time: docTime,
        doc_date_calc: docDate,
        doc_time_calc: docTime,
        last_status: 0,
        line_number: 0,
        ratio: 0,
        ...
        qty: diffQty,
        price: averageCost,
        price_exclude_vat: averageCost,
        sum_amount: diffCost,
        sum_amount_exclude_vat: diffCost,
        wh_code: finalWhCode,
        shelf_code: finalShelfCode,
        branch_code: finalBranchCode,
        average_cost: averageCost,
        average_cost_1: 0,
        sum_of_cost: diffCost,
        sum_of_cost_1: diffCost,
        is_get_price: 1,
        creator_code: approver.user_code,
        sale_code: approver.user_code,
      });
      ...
      const newBalanceResult = await client.query(
        `SELECT COALESCE(SUM(balance_qty), 0)::numeric AS new_balance
         FROM sml_ic_function_stock_balance_warehouse_location('NOW()', $1, '', '')`,
        [itemCode],
      );
      await client.query(
        `UPDATE ic_inventory
         SET balance_qty = $1
         WHERE code = $2`,
        [safeNumber(newBalanceResult.rows[0]?.new_balance), itemCode],
      );

      return {
        doc_no: adjDoc.doc_no,
        adjust_doc_no: adjDoc.doc_no,
        adjust_doc_format_code: adjDoc.doc_format_code,
        trans_flag: 66,
        item_code: itemCode,
        unit_code: unitCode,
        wh_code: finalWhCode,
    ...
    console.error("adjustStockFromSale error:", ex.message);
    return res.status(ex.statusCode || 500).json({ success: false, msg: ex.message, ERROR: ex.message });
  }
});
...
// POST /service/v1/getInventoryBalanceBatch
router.post("/getInventoryBalanceBatch", async (req, res) => {
  const { item_codes = [], wh_code = "" } = req.body || {};
  ...
  try {
    const result = await query(
      `WITH requested AS (
       ...
       SELECT r.item_code,
              COALESCE(SUM(s.balance_qty), 0) AS sum_balance_qty
       FROM requested r
       LEFT JOIN LATERAL (
         SELECT balance_qty
         FROM sml_ic_function_stock_balance_warehouse_location('NOW()', r.item_code, $2, '')
         WHERE balance_qty > 0
       ) s ON TRUE
       GROUP BY r.item_code
       ORDER BY r.item_code`,
      [codes, String(wh_code || "").trim()],
    );
    ...
    return res.json({
      ...
      data: result.rows.map((row) => ({
        ...
        sum_balance_qty: Number(row.sum_balance_qty) || 0,
      })),
    });
  } catch (ex) {
    console.error("getInventoryBalanceBatch error:", ex.message);
    return res.status(500).json({ ERROR: ex.message });
  }
});

// POST /service/v1/getProductWarehouseBalanceBranches
router.post("/getProductWarehouseBalanceBranches", async (req, res) => {
  const { selected_branch_code = "", item_code = "", ratio = 1 } = req.body || {};
  const selectedBranchCode = String(selected_branch_code || "").trim();
  const itemCode = String(item_code || "").trim();
  const unitRatio = Math.max(Number(ratio) || 1, 1);

  try {
    if (!itemCode) {
      const result = await query(
        `SELECT
           COALESCE(w.branch_code, '') AS branch_code,
           COALESCE(b.name_1, '') AS branch_name,
           CASE
             WHEN $1 <> '' AND COALESCE(w.branch_code, '') = $1 THEN 0
             ELSE 1
           END AS branch_sort,
           COUNT(w.code)::int AS warehouse_count,
           0::numeric AS balance_base,
           0::numeric AS balance_qty
         FROM ic_warehouse w
         LEFT JOIN erp_branch_list b ON b.code = w.branch_code
         GROUP BY COALESCE(w.branch_code, ''), COALESCE(b.name_1, '')
         ORDER BY branch_sort, COALESCE(w.branch_code, '')`,
        [selectedBranchCode],
      );
      ...
      return res.json({
        ...
        data: result.rows.map((row) => ({
          ...
          branch_sort: Number(row.branch_sort) || 0,
      ...
      `WITH warehouses AS (
       ...
       balances AS (
         SELECT
           ...
           wh.branch_sort,
           wh.wh_code,
           COALESCE(stock.sum_balance_qty, 0)::numeric AS balance_base
         FROM warehouses wh
         LEFT JOIN LATERAL (
           SELECT COALESCE(SUM(balance_qty), 0) AS sum_balance_qty
           FROM sml_ic_function_stock_balance_warehouse_location('NOW()', $2, wh.wh_code, '')
         ) stock ON TRUE
       )
       SELECT
         branch_code,
         branch_name,
         branch_sort,
         COUNT(wh_code)::int AS warehouse_count,
         COALESCE(SUM(balance_base), 0)::numeric AS balance_base,
         TRUNC(COALESCE(SUM(balance_base), 0)::numeric / $3::numeric, 0) AS balance_qty
       FROM balances
       GROUP BY branch_code, branch_name, branch_sort
       HAVING TRUNC(COALESCE(SUM(balance_base), 0)::numeric / $3::numeric, 0) > 0
       ORDER BY branch_sort, branch_code`,
      [selectedBranchCode, itemCode, unitRatio],
    );
    ...
    console.error("getProductWarehouseBalanceBranches error:", ex.message);
    return res.status(500).json({ ERROR: ex.message });
  }
});

// POST /service/v1/getProductWarehouseBalances
router.post("/getProductWarehouseBalances", async (req, res) => {
  const {
    item_code = "",
    unit_code = "",
    ratio = 1,
  ...
  const itemCode = String(item_code || "").trim();
  const unitCode = String(unit_code || "").trim();
  const selectedBranchCode = String(selected_branch_code || "").trim();
  const targetBranchCode = Object.prototype.hasOwnProperty.call(req.body || {}, "branch_code")
    ? String(branch_code || "").trim()
    : selectedBranchCode;
  const unitRatio = Math.max(Number(ratio) || 1, 1);
  const offset = Math.max(parseInt(first, 10) || 0, 0);
  const limit = Math.min(Math.max(parseInt(rows, 10) || 50, 1), 200);
  ...
  try {
    const result = await query(
      `WITH warehouses AS (
         SELECT
           ...
           COALESCE(b.name_1, '') AS branch_name,
           CASE
             WHEN $2 <> '' AND COALESCE(w.branch_code, '') = $2 THEN 0
             ELSE 1
           END AS branch_sort
         FROM ic_warehouse w
         LEFT JOIN erp_branch_list b ON b.code = w.branch_code
         WHERE COALESCE(w.branch_code, '') = $4
         ORDER BY w.code
       ),
       balances AS (
         SELECT
           ...
           wh.wh_name,
           COALESCE(stock.sum_balance_qty, 0)::numeric AS balance_base,
           TRUNC(COALESCE(stock.sum_balance_qty, 0)::numeric / $3::numeric, 0) AS balance_qty
         FROM warehouses wh
         LEFT JOIN LATERAL (
           SELECT COALESCE(SUM(balance_qty), 0) AS sum_balance_qty
           FROM sml_ic_function_stock_balance_warehouse_location('NOW()', $1, wh.wh_code, '')
         ) stock ON TRUE
       ),
       filtered AS (
         SELECT *
       ...
       SELECT
         ...
         p.total_records
       FROM paged p
       ORDER BY p.branch_sort, p.branch_code, p.wh_code`,
      [itemCode, selectedBranchCode, unitRatio, targetBranchCode, offset, limit],
    );
    ...
    console.error("getProductWarehouseBalances error:", ex.message);
    return res.status(500).json({ ERROR: ex.message });
  }
});
    ...
    const [countRes, dataRes] = await Promise.all([
      ...
      query(
        `SELECT i.code, COALESCE(i.name_1,'') AS name_1, COALESCE(i.name_eng_1,'') AS name_eng_1,` +
          ...
          ` FROM ic_inventory i${whereSql} ORDER BY ${orderBy} OFFSET ${offsetParam} LIMIT ${limitParam}`,
        dataParams,
      ),
    ]);
