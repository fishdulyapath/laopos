...
async function loadPromotionsFromDb({ includeInactive = false } = {}) {
  ...
  const [formula, condition, action, group] = await Promise.all([
    ...
    query("SELECT * FROM ic_promotion_formula_condition WHERE COALESCE(condition_from,'') <> '' ORDER BY code, COALESCE(roworder,0), COALESCE(line_number,0)", []),
    query('SELECT * FROM ic_promotion_formula_action ORDER BY code, COALESCE(roworder,0)', []),
    query('SELECT * FROM ic_promotion_formula_group_qty WHERE COALESCE(group_number,0) > 0 ORDER BY code, COALESCE(roworder,0)', []),
  ]);
