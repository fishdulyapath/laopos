const PERMISSIONS = [
  ...
  { key: 'dashboard.today_sales', label: 'แดชบอร์ด: ดูยอดขายวันนี้' },
  ...
  { key: 'sales.cash.view', label: 'ประวัติการขาย: เข้าหน้าจอ' },
  { key: 'sales.cash.detail', label: 'ประวัติการขาย: ดูรายละเอียด' },
  { key: 'sales.cash.print', label: 'ประวัติการขาย: พิมพ์' },
  { key: 'sales.cash.edit', label: 'ประวัติการขาย: แก้ไข' },
  { key: 'lao_qr.history.view', label: 'ประวัติรับเงิน QRLao: เข้าหน้าจอ' },
...
async function getEmployeePermissions(query, userCode) {
  ...
  try {
    ...
    if (result.rows.some((r) => r.permission_key === 'sales.credit.view' && r.is_allowed === true)) {
      allowed.add('sales.cash.view');
    }
